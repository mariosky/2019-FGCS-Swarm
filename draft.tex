...
many populations as part of their optimization strategy. % Some comment about asynchrony would be in order - JJ
...
success \cite{li2015multi,godio2016multi,biswas2014co}. % References - JJ
...
% Maybe this should be mentioned earlier, in the workflow? - JJ
...
changing the number of populations used in each run. %What did you
                                %want to prove? - JJ
The left column has the
results when using five populations, and the right side shows the results for
ten. % Why only 5 and 10? - JJ
     % mmm I really do not have an answer for that, I was thinking about 
     % testing from 1 to 15, and also more workers - Mario

Charts show the execution time achieved by a certain number of workers.
...
workers, using 5 populations is better in most cases. A 
...
1:1 between the number of workers and populations  better exploits the computing
resources available.
% I don't really understand this too well... JJ
% Also, it might just be that populations are waiting on the queue to
% be processed if there are more sub-populations than workers. The
% fact that 10 subpopulations take more with 1w than 5 subpopulations
% might be due to late startup as you say above, but you need to
% explain it better.

% I was thinking also in terms of the heterogeneous populations.
% If population 1 to 6 are exploiting, and 7 to 9 are exploring
% they will take longer to catchup. 
% While with more workers this happens almost at the start
% If it makes sense, I will try to explain it better tomorrow with a 
% clear head - Mario    

% Actually, what I see is no significant difference, or diminishing
% difference, between 5 and 10 sub-populations as the number of
% workers increases, and as long as the number of workers is less than
% the number of subpopulations.

% I think that was the point, but not too much less for instance 1w and 10pop
% Also I see a very big jump between 1w and 2w - Mario
...
%         \includegraphics[width=0.48\textwidth]{ppfigs_f001}&
%         \includegraphics[width=0.48\textwidth]{ppfigs_f002}\\

%         \includegraphics[width=0.48\textwidth]{ppfigs_f003}&
%         \includegraphics[width=0.48\textwidth]{ppfigs_f004}\\
        
%         \includegraphics[width=0.48\textwidth]{ppfigs_f005}&
%         \\
%         \end{tabular}
%         \vspace{-3ex}
%          \caption{Average running time (aRT in number of f-evaluations as $log_{10}$ value),
%           divided by dimension for target function value $10^{-8}$ versus dimension. 
%           Slanted grid lines indicate quadratic scaling with the dimension. 
%           Different symbols correspond to different algorithms given in the 
%           legend of $f_1$. Light symbols give the maximum number of 
...
%           all other algorithms with $p < 0.01$ and Bonferroni 
