...
netween having one or many copies of the same function pulling work from the queue all at the
same time, because no side effects are going to result from the concurrency.
Based un these general principles we define next the proposed model.
...
Figure \ref{fig:kafkEO}. Again, we can explain the general model using the analogy
of Producers and Consumers of messages using a queue. First, we can see that
