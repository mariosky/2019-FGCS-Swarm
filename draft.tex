...
suitable for hosting multi-population, multi-algorithm optimization algorithms. % reproducible? It's in the title - JJ Yes - M
...
stream of data. We show the differences of the message-driven architecture we presented against
...
% But it's not a practical comparison, but a theoretical one. Show the differences instead of compare? - JJ - Ok 
% We found that it is best suited for asynchronous multi-population algorithms. % This is not really proved. You chose, you didn't find - JJ
% Ok, I put it in the same package - M
...
researchers.  % and we proved... Remember the experimental hypothesis - JJ - Ok - 
We tested the deployment of an experiment on Amazon's Elastic
Container Service, and proved that as cloud-native, container-based application,
...
in terms of execution speed with experiments taking less time because
evaluations are shared between different workers. % of what kind? - JJ
...
for visualization and definition of the optimization problems. % as a visualization device? - JJ  
% we also used their functions -M
