...
master-worker approach, comunicated using RabbitMQ (a messaging
...
have chosen them in this paper; reproductibility of experiments and a
...
in Evo\-Swarm, and three roles users can play. The first step is to develop the 
search strategy algorithms that are going to be used by the multi-population algorithm.
...
have additional needs. As a use case, there are additional scripts for
processing the results of experiments to be used with the COCO framework. In the
next sections, we give more details about these roles and their workflow.  
