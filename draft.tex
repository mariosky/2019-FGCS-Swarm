...
% I will add a comment about this in the Multi-population section - Mario
...
a message, and then send the message asynchronously % I don't know if
                                ...
                                % simply sent - JJ 
                                % Meaning it does not wait for a respone, it continues doing other stuff - Mario 
...
\begin{sidewaysfigure}
    ...
    \caption{The proposed general architecture, 
     ...
     message queues, and high-level tasks for each process.} % This caption? - JJ
    \label{fig:kafkEO}
\end{sidewaysfigure}
...
pass to the pay-as-you-go tier, computing resources need to be managed.  First,
in many cases, there is a credit-card need to guarantee the payment of the
...
value, or the error of the best solution reaches a specific size.  Finally, it
...
specialized in asynchronous event-driven programming over data streams.  The
open-source library is called Reactive Extensions for Python (RxPY)
\url{https://github.com/ReactiveX/RxPY} based on the Observer pattern
\cite{gamma1995design} and functional programming.  
...
generate the files needed by the COCO framework, which can generate standard
