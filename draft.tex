...
{\texttt setup} (128),  {\texttt controller} (896),{\texttt redis} (1024). We assigned 2048 units to 
each {\texttt worker}   container, by using a more capable {\texttt C5 High-CPU XLarge} instance,  with 
four vCPUs and 4096 cpu shares. We scaled the number if instances as needed, using one instance
for every two {\texttt worker} containers. Because all these instances run in a private subnet, we
run a Bastion instance in a public subnet, from where we executed the experiments and collected the results,
through a {\texttt ssh} session. This is a {\texttt R5AD Large} instance,  with 75GiB of storage for
storing the logs, and more memory (16GiB) for processing the logs. The details and cost of these instances at the 
time of writing are shown in Table \ref{tab:ec2}.       
...
This configuration files will be
used on the next section, to carry an extensive evaluation of the framework.
...
We have deployed the EvoSwarm application in Amazon Elastic Container Service as described in Section~\ref{cloud-aws}, 
