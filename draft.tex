...
to have the same effect. Regarding GAs using a similar architecture, the work of
Salza et al., suggest that worker nodes, need to have at least a certain level
...
For the experiment, we selected $f_4$ (Skew Rastrigin-Bueche separable) from the
BBOB testbed. We selected this function because it is computationally demanding,
and in higher dimensions has been difficult for PSO \cite{el2009black} and GAs
\cite{nicolau2009application} to solve with the required FEs. As we are
comparing just in terms of execution time, the results from a single function
can be better understand, because there are fewer factors involved. 
...
populations increases the performance of the search by needing fewer function
evaluations than a homogeneous setting.? 

In this experiment, we compare the performance of both heterogeneous and
homogeneous settings. We compare a multi-population with only  GA and PSO
populations, versus an ensemble of GA and PSO algorithms on the first five
functions of the BOBB testbed.  We used ten populations and 8 workers for the
experiment and the same parameters as before.  
