...
instances executing at the same time, not only in a single machine, not only 
in a single machine as it is the case when using docker-compose. 
...
\begin{table}[h!tbp]
  \caption{Speedup by worker and dimension, taking one worker as the
    baseline. Speedup is obtained by dividing the evaluation ratio of the baseline by the 
...
% Clarify if they actually achieve that way superlinear improvements - JJ
% They should, in this experiment we only tested the #FEs, but they imply 
% a superlinear improvements as we established in the previous section - Mario 
...
containers, and algorithm selection. Further research should focus on learning
...
% Probably a final conclusion should be added here, including BBOB
% benchmark and the overall performance - JJ
