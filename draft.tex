...
In this paper, we have presented the EvoSwarm framework that implements an
event-based, cloud-native architecture suitable for hosting multi-population % multi-algorithm? - JJ
optimization algorithms. % reproducible? It's in the title - JJ
...
multi-algorithm distributed experiments. The model is based on the asynchronous
exchange of messages between stateless functions that react to a continuous
stream of data. We compared the message-driven architecture we presented against
four other proposals for multi-population algorithms found in the literature. % But it's not a practical comparison, but a theoretical one. Show the differences instead of compare? - JJ
We
found that it is best suited for asynchronous multi-population algorithms. % This is not really proved. You chose, you didn't find - JJ
...
definition in both local or cloud environments. We presented the workflow of an
experiment's entire life-cycle viewed from the perspective of developers and
researchers. We tested the deployment on both local and Amazon's Elastic
Container Service. % and we proved... Remember the experimental hypothesis - JJ
...
populations slightly above the number of workers, which brings performance gains. % of what kind? - JJ
...
Optimization (PSO) using the COCO benchmark framework for continuous functions. % as a visualization device? - JJ
