...
in a loop. The \texttt{Controller} is also responsible of logging the metadata recived with the
messages.  
\end{enumerate}
...
The same basic components can be implmented as a single multi-threaded program, or 
...
possibility of a serverless-function implementation.   
...
queues in the Redis memory store. Each queue is a Redis List object, and we use
...
\texttt{BRPOP}. Redis in-memory operation are very fast, and all of the obove 
...
is a repository of more than 200 results for the noiseless BOBB testbed,
collected from BBOB workshops and special sessions between the years 2009 and
2019 \url{https://coco.gforge.inria.fr/doku.php?id=algorithms-bbob}. The
EvoSwarm application includes an adapted version of the noiseless BOBB testbed,
compatible with the scripts of the framework, to compare with other algorithms
in the repository.

To validate the heterogeneous multi-population setting, we compare between
homogeneous and an ensemble of multi-populations, using Genetic Algorithms
(GAs) and Particle Swarm Optimization (PSO). For the GA implementation, we used
the DEAP library \cite{fortin2012deap} and for the PSO, the EvoloPy library
\cite{faris2016evolopy}. Both Python libraries are open-source. In EvoSwarm 
these two algorithms are implemented as stateless functions. 


We show the parameters for each algorithm, GA in Table \ref{tab:GAparams} and
for PSO in Table \ref{tab:PSOparams}. We obtained these parameters in a
previous work \cite{}. To obtain the parameters, we tested first on the
