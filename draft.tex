...
declarative manner. In Figure /ref{marble}, we use a so-called marble diagram to
...
In Figure /ref{marble}, we  have the following composition of Observables:

\begin{enumerate}

\item The controller continually pulls new messages from the Output 
message queue. These messages are instantly emitted by the consumed\_messages 
...
\item The max\_iterations parameter indicates the number of populations that
are going to be accepted. When this number of messages is reached, we must
end the loop. In this example, max\_iterations = 6,  the take operator 
assures that only 6 messages are received. After the Observable emits 
the sixth message, it triggers the completed event.

\item Many observers can be subscribed to the valid\_messages Observable because it
...
\item The buffer\_with\_count(3) operator, waits until it receives 
...
\item The population\_mixer method pushes new populations to the messages Observable,
a publish(population) observer is responsible for pushing 
