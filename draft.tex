...
Developers can add new algorithm stateless functions in
...
docker container that executes a demon that continuously pulls messages from the
...
the provided worker container image, adding a new type of function.  
...
as a parameter. Depending on the implementation, it required to extract the
...
can be returned in the message. For instance, the message can include the id of
the container, or even records giving information for each iteration of the
algorithm. The format of the JSON message is depicted in Figure y. % Adding to Annex or is not even needed? - M

For the next level of integration, developers need to add code to the setup
container to specify additional configuration options that can be related to
other workers.

Migration between populations it is independent of the type of algorithm, as the
...
The first step is to develop the search strategy
...
configuration files can reference these images. When a user needs to deploy
...
Container Service (ECS).  Because it needs fewer changes in configuration, 
