...
In the case of the first level of integration, a decoupled worker, % a decoupler worker what? - JJ
developers
can implement a new stateless Docker container following the description in the previous section (\ref{workers}). The worker
...
% Anyway you need to say what is the signature of the function. Also, if they can use the existing implementation, you need to specify the license - JJ
...
population because, in this case, the algorithm need to start with the population provided
as a parameter. Depending on the library, % Depending on which library? - JJ
...
to track the execution of the algorithm and keep a log in the Redis 
container for posterior analyses. 

\begin{lstlisting}[language=JSON, caption=Fragment of output message, label=code:result ]
{
