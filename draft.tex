...
%and only come into existence while they are doing some processing.
...
method's effectiveness, concerning scalability and % This is a subsection, fix it. 
...
by those selected in other populations \cite{li2016multi,tanabe2013evaluation}. % reference - JJ
...
%On the other hand, containerization also
...
%different parts of the application, or services. 
% We said this earlier - Mario
...
%, and also automation of the whole workflow. 
...
%optimization problems with 1000 dimensions.  However, in other cases, % Reference that paper - Mario
...
After starting the application, users can push several Experiment
Definition files and monitor the current execution using the PC console.
After an experiment is finished, the user must execute
another script that takes as argument the experiment id to generate a
collection of files containing all the data generated by the
experiment, again in a JSON format. An example of an experiment
definition is shown in Listing \ref{code:exp}. We give more details
about the data contained in the file in the next section. Users can
use these files to analyze and plot experiment data. We include Python
scripts in the repository to plot the running times for the
experiments and all other charts used in this paper; there is also a
script to generate the files needed by the COCO framework
\cite{hansen2016coco}, which can generate standard comparisons against
other methods.
...
benefit of a multi-instance deployment. We can run containers in several
computing nodes at the same time, not only in a single machine as it is the case when using docker-compose.
...
since it needs fewer configuration changes (from deploying it locally using
...
Multicontainer applications are deployed to ECS as a series of tasks {\em tasks},    and a single cluster can run multiple tasks in
parallel. Using the ECS CLI, we can provision a cluster with a configuration file
specifying the computing resources needed. We can also specify a task's configuration 
...
The complete task configuration is generated on-the-fly by the {\texttt ecs compose command} 
that uses a {\texttt docker-compose.yml} file to generate 
the configuration. For example, to deploy the controller, setup, and Redis 
containers, we used the file in Listing ~\ref{code:compose}.

Each container is defined as a {\texttt service} (a {\em task} in AWS terms), with the following attributes: {\texttt image},
that reference the container images we have published in Docker hub
\footnote{They are shown as different repositories for user {\tt
    mariosky} here: \url{https://hub.docker.com/u/mariosky}}, open {\texttt ports}, a {\texttt command} to be executed at
...
resources and we do not need to scale them as the number of parallel
workers is increased. They run in a single {\texttt C5 High-CPU
...
gets a dedicated vCPU because it is the most demanding task.
We assigned 2048 units to each {\texttt worker} container
and used a more capable {\texttt C5 High-CPU XLarge} instance, with
...
Bastion instance, a specially protected network node, in a
public subnet. From this instance, we send the commands to execute the
experiments and later collect the results. We connected to this
instance through an {\texttt ssh} session from our workstation.
This is an {\texttt R5AD Large} instance, selected with
75GiB of storage, needed for keeping the logs, and more memory (16GiB)
for processing the logs in memory. The details of these EC2 instances and
...
The complete configuration files are available at public the GitHub repository 
...
and portable way. It was used in each phase of the life cycle, from
...
population-based optimization algorithms' scalability and performance.
Hence, in the following sections, we provide answers to
the following questions:

\begin{itemize}
\item Can we improve the algorithm's execution time by adding populations
 and serverless functions (workers)  to the system and, in particular,
 what is its effect on the system's scalability? % Improve with respect to what? Single population? 
...
the support for mixing search strategies,  increase the search's
performance by needing fewer function evaluations than a homogeneous setting? 
...
value. The standard benchmark runs 15 instances per function
...
The COCO framework offers several tools to compare algorithms' performance,
generating data sets, tables, and reports for an experiment. There
...
Genetic Algorithms (GAs) and Particle Swarm Optimization (PSO). 
We used the DEAP library \cite{fortin2012deap}, for the GA implementation. 
For the PSO algorithm, the EvoloPy library \cite{faris2016evolopy}. Both Python libraries are
open-source. In EvoSwarm, we implemented these two algorithms as stateless
functions.
...
Next, we show each algorithm's parameters, with Table
\ref{tab:GAparams} for the GA and Table \ref{tab:PSOparams} for the
PSO. We obtained these parameters following the same method as in 
...
five dimensions.  After about fifteen experiments, the algorithm 
achieved the most challenging targets for this particular function. We tested again
with functions one to three, and after obtaining favorable results, 
we set the PSO and GA parameters to these values. In the GA, we randomly
set the mutation and crossover probabilities to have more
heterogeneous workers; for these parameters, we specify the range 
of values in Table \ref{tab:GAparams}. We did not change these 
parameters during the experiments and only modified the number of
populations, population size, and generations. This strategy has been
...
files described in Section~\ref{sec:evoswarm:config}. 
...
functions we will test. In the example, the experiment will use the first five
functions.  The {\em Instances} parameter indicates how many instances of each
function we will teste. {\em Instances}  have a default value of 15. We used this
...
earlier, for each dimension, we define the number of populations that will be
...
complete loops, that is, the maximum number of messages taken from the queue.
All these parameters give us the maximum
number of FEs we will have as budget. For instance, for $D = 2$, the maximum
...
in Python, and the plots were produced with version 2.3.2.
...
of complexity, in terms of execution time, to scale on multiple
nodes, tending to linear scalability. In that paper, Salza et al. tested scalability
by simulating the nodes' work by using a sleep function and messages of different sizes.
...
From this description, we can deduce that we give more importance 
to the number of populations since they are the basic unit of
work, the message, that can be executed in parallel. Together with each population's size and the
number of generations, this parameter determines the maximum number of function
evaluations for a single iteration. According to this, we do not have control 
over each algorithm's complexity, but future analyses could focus on balancing 
populations and the number of worker nodes.

In this experiment, we evaluate how the number of populations and workers 
are related to the system's scalability.
In the next section, we describe the experimental setup.A
discussion of the results follows, concerning the speed-up in
...
number of populations and the number of workers on the system's scalability.
To achieve this, we propose an experiment for
which we have selected $f_4$ (Skew Rastrigin-Bueche separable) from the BBOB testbed,
over 10 and 20 dimensions. This function has been used
because it is computationally demanding, and in higher dimensions, it has been
difficult for PSO \cite{el2009black} and GAs \cite{nicolau2009application} to
solve with the required number of FEs. As we are only comparing in terms of scalability,
the results from a single function can be better understood since fewer factors are involved.
Following the procedure described in Section~\ref{setup}, we executed two sets of 
experiments with five, ten, and twenty initial populations, repeating each 
experiment 30 times using 1, 2, 4, 8, and 16
...
\begin{figure}[h!tbp]
  ...
  \caption{Boxplot of the number of evaluations per second, for 30 instances of function $f_4$,
          with a different number of workers and populations. The $y$-axis is in log scale.}
...
instance of a problem. This is because in some cases reaching the
...
\begin{table}[h!tbp]
  \caption{Speed-up by worker and dimension, taking one worker as the
    baseline. Speed-up is obtained by dividing the baseline's evaluation ratio by the 
...
%
As expected, there is a linear speed-up when we add more workers 
within the same number of populations. The speed-up is marginal in those cases where there 
are fewer populations than workers. This is because, at any given time, 
...
the previous configuration, such as increasing from four to eight
...
basic rule doubled the speed or where near that. Using five populations, we
...
populations and workers, we increase the number of evaluations per
...
populations, eight workers will need fewer FEs than one (p = 0.012). Also, for
...
resulting from work parallelization.
...
The reduction in the number of FEs impacts when we measure the speed up
...
problem. Figure \ref{fig:spworker:time} shows in each boxplot the median (30
...
In Table~\ref{tab:time} we can see that we have a supralinear
speed-up in some cases; we highlighted these in boldface and indicated the speed-up value in
...
thanks to running the algorithm on several populations simultaneously.
However, if the populations are heterogeneous 
it might enhance the evolutionary search and avoid unneeded
...
Nevertheless, this is a rule of thumb, and it will depend on the degree of heterogeneity and 
the algorithm itself. Some level of heterogeneity can be 
achieved by just changing the configuration parameters of each population, 
as in the previous experiment.However, in this case, we are interested in answering 
the second research question. We want to validate if we can increase the search's performance 
using heterogeneous populations and mixing search strategies.
...
To run this experiment, we just needed to edit a configuration file and
...
needed, proving how this framework can offer a reproducible way
to run experiments and how easily it interfaces with other common tools in
the EC ecosystem.
...
reached the $10^{-8}$ target on all instances for that dimension. If there
is a number, it indicates how many times the algorithm has reached
the target. Fixed values of targets $\Delta f = 10^{k}$ with $k$ colors are
given in the $f_1$ legend in the top row, $k$ values are $[-8,-5,-3,-2,-1,0,1]$. Results
...
On the other hand, the multi-population PSO (taking the column in the middle) 
...
$10^{-8}$ target on higher dimensions of $f_3$ and $f_4$. Moreover, the PSO
...
value on all functions ($f_1-f_5$). It scales nicely to higher dimensions, even on
$f_3$ and $f_4$ (charts on the lower rows), usually considered
difficult functions. This experiment also exemplifies the type of analyses that
we can perform with the EvoSwarm application results by reusing the COCO framework.
We also proved that multi-population algorithms could be compared under the same 
conditions, including single and multi-algorithm versions, and results show 
that the latter has the best performance.

\section{Conclusions} 
\label{conclusions}
This paper has presented the EvoSwarm framework for the
...
literature and highlight the advantages of conducting asynchronous
...
solution can efficiently improve population-based optimization algorithms' 
...
(experiments are taking less time because they need fewer evaluations) of
...
Swarm Optimization (PSO), using the COCO benchmark framework to visualize and define 
optimization problems.  We found
that these algorithms' set-up has a significant influence on
...
containers, and algorithm combinations. Further research should focus on learning
...
auto-scaling capabilities, but additional configuration is needed; we also need
...
would be a possibility and using different instances of
...
which could be tested, trying to determine which sets of algorithms are
a better match for each other. We will explore all this as future lines of work.
