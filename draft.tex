...
on the Redis host specified as an environmental variable REDIS_HOST. If the new
...
% say which function, what's its API, how can you test it - JJ
% this is not an API the need to write their own - Mario
% We can write an API later - M
...
as a parameter. Depending on the library, % Depending on which implementation? - JJ
...
must be returned in the JSON message, % how do you know the type of experiment? how do you add that? Which JSON key? - JJ
% I was wrong, it does not depend on the problem. - M
...
container for posterior analyses. . % Is this mandatory? Optional? How is the algorithm going to use it? - JJ
% Mandatory and used for everything - M
...
container to specify additional configuration options. For instance,  % related to? What do you mean here?
...
this range on the experiment configuration.  % Not clear how you integrate this with the rest. An example would probably help. - JJ
% done - M
...
For example, in the case of swarm-based algorithms,  migration between  % Added - M
...
In either level, algorithm developers can upload the container definition or images directly to
...
can be defined in DockerHub (or any other registry) too. 

% This is probably accessory. What difference would it make to upload it to GitHub?
% You also need to say in this section how it helps your overall objective. 
% Does it contribute to reproducibility? We've talked about that elsewhere. 
% Does it help diversity by making easy to add new algorithms? - JJ
...
data generated by the experiment in a JSON format as shown in Listing \ref{code:exp}.

\begin{lstlisting}[language=Python, caption=Experiment configuration example, label=code:exp]
{
"DIM_CONFIGURATION" : 
 {
...
We present the results of the experiments in this section. % that's pretty obvious. Say what you intend to prove with the experiments and why - JJ
% The effect of a
...
\begin{table}[h!tbp]
    \small
    \caption{Median evaluation rate per second, for each number of workers and dimension}
...
\begin{figure}[h!p]
    ...
    \caption{Boxplot of the number of evaluations per second, for 30 instances of the function, with different number of workers and populations sizes }
    \label{fig:spworker}
\end{figure}
...
\begin{figure}[h!p]
    ...
    \caption{Boxplot of the total number of evaluations for 30 instances of the function, with different number of workers and populations sizes }
    % Which function? - JJ
    \label{fig:spworker}
\end{figure}
...
%
% First you have to say that cloud is a way of describing deployment to any kind of infrastructure in a reproducible way, and it needn't be in the cloud proper, it can be deployed to anything, that's precisely the thing. But make clear that when you use a cloud provider, there are more options to do it... - JJ
Several options:
