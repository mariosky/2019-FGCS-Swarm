...
% Probably it's not the time for this, but maybe it would be interesting to mention that asynchronous implementations in most cases need a change in the underlying algorithm, while synchronous might not need it - JJ
...
arrive to the synchronization point; results arriving from other workers are incorporated as soon as they are received. Following the previous example, now the master operation can
...
% I have moved these sentences here, since they seem to fit the best.
...
a search strategy on several populations, % clarify "on several populations" Sequentially? At the same time? - JJ
...
application in Section \ref{docker} . % No reference to section 6.
Section \ref{setup} describes
the design of the empirical evaluation we designed to assess the effectiveness
of the method, and in Section \ref{results}, % This is a subsection, fix it. 
we report and discuss the results. % No references to sections 9 and 10
Finally, we
...
we can perform with the results of the EvoSwarm application by reusing the COCO framework. % If this is what you want to prove, it's not a very strong point. Maybe it would be stronger if you proved that these mixed algorithms can be tested in the same conditions as single-algorithm, and also that these mixed algorithms work better than single-algorithm versions - JJ

\section{Deploying to a cloud provider}

% This should probably be a subsection of section 5, where you describe what any user needs to do to get this running and you  talk about reproducibility - JJ
...
containers we used the file in Listing ~\ref{code:compose}. We have 
provided the container images and are published in Docker hub \url{https://hub.docker.com/u/mariosky}.
...
\end{lstlisting}
% Do you need the details about the subnets? As long as they are here, you'll have to explain them. Why two subnets? It's probably better to leave that network configuration out anyway - JJ
