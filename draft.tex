...
% Be consistent with docker compose. Either {\tt docker-compose} or {\sf docker-compose}, but not docker compose, for instance. - JJ - ok
...
As a proof-of-concept, we present a brief experiment that is deployed in three
distinct environments, taking advantage that as a container-based application,
EvoSwarm describes the deployment of loosely coupled processes in a consistent
and portable way.   

The term cloud-native describes these container-based
environments because we can deploy them on an infrastructure that abstracts the
underlying compute, storage, and networking primitives. This infrastructure does not
need to be a cloud provider; it could be a developer's laptop, a single server,
or a production environment. When we deploy a cloud-native application to a
cloud provider, we have some more options to choose:
...
% Mention cost here - JJ
% I putted earlier literally at the end of the section :) - M  
  
\subsubsection{Experimental Results} 

In this experiment, we wanted to test several deployment options for the 
cloud-native application, and have an idea of the computing capacity of each option
measured as the number of function evaluations per second.
...
ratio of evaluations per second (in bold). Still, we must consider that there is minimum
...
{\tt docker-compose} file, and one command to execute. But with the limit of 4 CPUs and
ten containers, it is the slowest option. The second option (labeled Multi), is
faster, and not too far from the local execution, especially when solving the
$f_4$ in 20 dimensions (underlined in table). With the capacity for increasing the number of worker
...
flexibility of a cloud-native application. The total cost of testing, configuration, 
and execution of the experiments in ECS was less than \$1.00 US Dollar.
...
From these parameters, we give more importance in this paper to the number of
populations, because they are the basic unit of work that can be executed in
parallel. Together with the size of each population and the number of
generations, this parameter determines the maximum number of function
evaluations for a single iteration. We do not have control over each algorithm's
complexity, but future analyses could focus on balancing populations according
to this.  
...
To answer the first research question, we need to evaluate the effect of 
the number of populations and the number of
workers on the scalability of the system, we 
...
are only comparing in terms of scalability, the results from a single function
can be better understood, since there are fewer factors involved.
Following the procedure described in Section~\ref{setup}, we executed 
...
A reason for this could be that when having more populations, the migration
of individuals takes more time to reach to all of them.
% how come? - JJ  - Explained a bit more - M
For instance, when a single worker has 10 populations, only
after processing every one of them, it will start receiving mixed populations.  Although, in
all cases, and even with a single worker, populations are heterogeneous, but
mixed individuals take more time to propagate. % which advantage? - JJ - Individuals from other pops - M
With four workers, almost the same
...
general, we can observe that keeping a ratio between the number of workers and populations a bit over 1:1  exploits better the computing
...
workers, for every problem dimension, for five and ten dimensions of $f_4$.  % Why did you choose f4? - JJ This is mentioned on the setup - M
...
\begin{table}[h!tbp]
  ...
  \caption{Speedup achieved with incrementing the number of workers for
    each dimension showing the p-rate for the Wilcoxon test.}
...
workers, and the p-rate for the left-tailed Wilcoxon test. There was an
increment in all cases, but again, for smaller dimensions, five populations with
fewer workers is a better option (see values underlined in the table) with the best speedup, achieved for 40 dimensions, shown in bold. The worst speed was going from four to eight workers 
at higher dimensions (underlined in the table). 
For ten populations, adding more workers resulted in a linear speedup, % don't understand this - JJ % Me either - M, rewritten  
but the best speedup is obtained when increasing from four to eight workers at 40 dimensions
(shown in bold).


% No idea where this fits in. I think it's the second question in 7, 
% so it should probably be a subsection - JJ
% done - M
...
In order to run this experiment, we just needed to edit a configuration file and run 
the experiments. The plots presented in the results are generated by the BBOB framework
from the JOSN log file of each run. There was no additional setup needed. 
